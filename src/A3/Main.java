...
After the employee classes are finished, students should write a PayrollSystem class that stores a list of Payable objects. This class needs a method called processPayroll that loops through every employee and prints their pay details, along with a method called totalPayrollCost that adds up the result of calculatePay across the entire list.

Students should write a test class that builds a payroll system, adds a few employees of different types, and runs payroll. At the top of that test class, as a comment, students should answer the following question in three or four sentences: why is Employee written as an abstract class while Payable is written as an interface, and what would be lost if Payable had been written as an abstract class instead? Every class and interface should be placed in its own file.
 */
