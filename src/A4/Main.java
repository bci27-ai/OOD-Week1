...
/*
Students will build a system for a vehicle rental company that manages a fleet of vehicles, rents them to customers, and calculates rental costs based on the type of vehicle.
...
Next, students should write an abstract Vehicle class
    ...
    - specifically make, model, year, and dailyRate. This class should include a constructor that sets all four fields and a concrete method called displayVehicleInfo that prints them.
Vehicle should not implement calculateRentalCost or getRentalTerms, since those behaviors come from the Rentable interface and need to differ from one vehicle type to the next.
...
Each subclass needs its own version of calculateRentalCost that reflects a realistic pricing rule for that vehicle — for example, a truck might charge a higher daily rate plus a flat mileage fee, while a motorcycle might charge a lower daily rate but add a mandatory insurance fee.
